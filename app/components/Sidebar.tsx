...
export default function Sidebar({ active }: { active: string }) {
  ...
  useEffect(() => {
    ...
    const chargerStats = () => {
      supabase.from("profiles").select("avatar_url,role,statut,last_seen").then(({ data }) => {
        ...
        const pourcentage = 0.02 + ((seed % 4) / 100);
  ...
  return (
    <>
      ...
      <aside className={`w-64 bg-gray-900 border-r border-gray-800 flex flex-col fixed z-40 transition-transform duration-300 ${sidebarOpen ? "translate-x-0" : "-translate-x-full"} md:translate-x-0`} style={{height:"100svh"}}>
