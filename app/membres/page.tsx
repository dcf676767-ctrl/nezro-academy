...
export default function Membres() {
  ...
  const load = async () => {
    ...
    setAdmins(avecStatut.filter((m:any) => m.role === "admin"));
    setLoading(false);
  };
  useEffect(() => {
    load();
        ...
        <div className="max-w-6xl mx-auto flex gap-8">
          <div className="flex-1">
            ...
            {loading ? (
              ...
              <div className="flex flex-col gap-4">
                {tous.map((m, i) => (
                  <div key={m.id} className={`bg-gray-900 border rounded-2xl p-5 flex items-center gap-4 transition-all ${m.enligne ? "border-blue-500/40" : "border-gray-800"}`}>
                    <Avatar m={m} />
          ...
          <div className="w-72 flex flex-col gap-4 flex-shrink-0 mt-24">
            ...
            <div className="bg-gray-900 border border-gray-800 rounded-2xl p-5">
              ...
              {enLigne.length === 0 ? (
                <p className="text-gray-500 text-sm">Personne en ligne pour l'instant</p>
              ) : (
